...
array reference or a L<List::Ish> object.  If it is an array
reference, it is blessed with the class.  If it is already blessed
with the class, it is returned as is.
...
array blessed by List::Ish using CORE::map() and returns the
results as a list or List::Ish object along with the context.
...
Executes C<$code> with each element of C<$self>, a refernce to an
array blessed by List::Ish using CORE::grep() and returns the
results as a list or List::Ish object along with the context.
...
List::Ish object along with the context.
...
Sorts out each element with Schwartzian transform returns the result
as a list or List::Ish object along with the context.

  $list = List::Ish->new([ [3], [2], [4], [1]]);
...
List::Ish, and returns the result.

  $list = List::Ish->new([1, 2, 2, 3, 3, 4])
  $list->uniq; #=> [1, 2, 3, 4]
...
<at> shibuya <döt> plE<gt>.  They acknowledge naoya, kentaro,
